...
public:
    ...
    void insert(const iterator &it, const std::pair<K, V> &key_value) {
        ...
        while (x != nullptr) { 
            ...
            if ((find(key).node_ptr != NULL) && find(key) != end()){//dupicate
...
private:
    ...
    void delete_tree(Node<K, V> *n) {
        // TODO
        if (n != nullptr){ //if there is a node
        ...
        while ((z->parent !=nullptr) && (z->parent->color == RED)){
            ...
            if (z->parent == z->parent->parent->left){ //if z.parent is left child
                ...
                if ((y !=nullptr) && (y->color == RED)){ //case 1
                    ...
                    if (z == z->parent->right){ //case 2 z's uncle is black z is currently right child
                        z = z->parent;
                        left_rotate(z);
                    }
                    z->parent->color =BLACK;  //case 3 z's uncle is black
                    z->parent->parent->color = RED;
                    right_rotate(z->parent->parent);
                }

            }
            //same as above but z is swapped left
            else{
    ...
    void left_rotate(Node<K, V> *x) {
        // TODO
        Node<K, V> *y = x->right; 
        x->right =y->left;  //make y left into x's right
        if (y->left != nullptr){
            y->left->parent=x; 
        }
    ...
    void right_rotate(Node<K, V> *x) {
        ...
        Node<K, V> *y = x->left;
        x->left= y->right; 
        if (y->right != nullptr){//exact opposite of left rotate
            y->right->parent = x;
        }
        y->parent=x->parent;
        if (x->parent == nullptr){
    ...
    int height(Node<K, V> *node) const {
        ...
        if (node == nullptr){
            ...
            //use the larger value for the height
    ...
    size_t leaf_count(Node<K, V> *node) const {
        ...
        //right and left are empty then it is a leaf
    ...
    size_t internal_node_count(Node<K, V> *node) const {
        // TODO
        //(total nodes)-leaves of tree =nonleafs
    ...
    int diameter(Node<K, V> *node) const {
        // TODO
        //If root = null
        if (node == nullptr){ //check empty
            return 0;
        }
        else {
            //longest path that goes true the root of the tree 
            int leftH = height(node->left)+1; //traverse the left and right side of the tree
            ...
            return std::max(rightH+leftH, std::max(left_diameter, right_diameter));
        }
    ...
    size_t width(Node<K, V> *node, size_t level) const {
        ...
        if (level == 0){
            ...
            //get the width of the specific level 
    ...
    size_t null_count(Node<K, V> *node) const {
        ...
        //else add the null count of both sies
        else {
    ...
    size_t sum_null_levels(Node<K, V> *node, size_t level) const {
        ...
        if (node ==nullptr){
            ...
            //every increase level we can increment both sides by 1
